using Ambulance.StatisticsApi.Client.Extensions;
using System;
...
namespace Ambulance.StatisticsApi.Client.Resources
{
        ...
        where T : class, IBaseContract
    {
        ...
        protected async Task<byte[]> GetBytes(Func<CancellationToken, Task<HttpResponseMessage>> action, CancellationToken cancellationToken = default)
        {
            cancellationToken.ThrowIfCancellationRequested();
            using var operationResult = await action(cancellationToken).ConfigureAwait(false);
            if (!operationResult.IsSuccessStatusCode)
            ...
            return await operationResult.DeserializeBytes(cancellationToken).ConfigureAwait(false);
        }
    }
}
