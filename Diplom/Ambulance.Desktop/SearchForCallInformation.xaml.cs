    ...
    public partial class SearchInformationAboutCall : Window
    {
        ...
        private async void ButtonSearch_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    CallNumber = string.IsNullOrWhiteSpace(callNumber) ? null : int.Parse(callNumber),
                    FIO = string.IsNullOrWhiteSpace(fio) ? null : fio,
                    Street = string.IsNullOrWhiteSpace(street) ? null : street,
                    HouseNumber = string.IsNullOrWhiteSpace(houseNumber) ? null : houseNumber,
                    FlatNumber = string.IsNullOrWhiteSpace(flatNumber) ? null : flatNumber,
                    Diagnosis = string.IsNullOrWhiteSpace(diagnosis) ? null : diagnosis,
                    Result = string.IsNullOrWhiteSpace(result) ? null : result,
                    DateFrom = dateFrom.IsEnabled ? DateTime.Parse(dateFrom.Text) : null,
                    DateTo = dateTo.IsEnabled ? DateTime.Parse(dateTo.Text) : null,
